...
def process_data():
	project_names=['cloud_controller_ng.csv']
	...
	for project in project_names:
		...
		for commit in range(len(X_test)):
			commit_values.append(final_data[34][commit])
			build_ids.append(final_data[0][commit])

		#print(commit_values)
		headers = ['Index','Duration', 'Build_Result']
		...
		with open(file_name, 'w+') as file:
			...
			for c_index in range(len(Y_result)):
				...
				writer.writerow([c_index+1, commit_values[c_index], Y_result[c_index]])
